...
class DataTrainingArguments:
    ...
    test_file: Optional[str] = field(default=None, metadata={
                                     "help": "A csv / tsv / jsonl file containing the test data."})
    do_normalize: Optional[bool] = field(
        default=True, metadata={"help": "Normalize text before feeding to the model."})
    unicode_norm: Optional[str] = field(
        default="NFKC", metadata={"help": "Type of unicode normalization"})
    ...
    def __post_init__(self):
        if self.train_file is not None and self.validation_file is not None:
            ...
            assert train_extension in [
                "csv", "jsonl", "tsv", "json"], "`train_file` should be a csv / tsv / jsonl file."
...
def main():
    ...
    def has_ext(path): return len(os.path.basename(path).split(".")) > 1
    def get_ext(path): return os.path.basename(path).split(".")[-1]
    ...
    for data_type, ds in raw_datasets.items():
        ...
        ignored_columns = set(ds.column_names) - set(
            [data_args.sentence1_key, data_args.sentence2_key, data_args.label_key])
    ...
    label_to_id = config.label2id if config.task_specific_params and config.task_specific_params.get(
        "finetuned", False) else None
    ...
    if data_args.do_normalize:
        ...
        def normalize_example(example):
            ...
            example[data_args.sentence1_key] = normalize(
                l, **normalization_kwargs)

            if data_args.sentence2_key in example:
                l = example[data_args.sentence2_key]
                example[data_args.sentence2_key] = normalize(
                    l, **normalization_kwargs)
    ...
    def preprocess_function(examples):
        # Tokenize the texts
        args = (
            (examples[data_args.sentence1_key],) if data_args.sentence2_key not in examples else (
                examples[data_args.sentence1_key], examples[data_args.sentence2_key])
        )
        result = tokenizer(*args, padding=padding,
                           max_length=max_seq_length, truncation=True)

        if label_to_id is not None and data_args.label_key in examples:
            result["label"] = [label_to_id[l]
                               for l in examples[data_args.label_key]]
    ...
    if training_args.do_predict:
        ...
        prob_df.to_csv(
            "/content/drive/MyDrive/Research/Shared Task/Violence Inciting Text Detection (VITD) Bangla/models/banglabert/probabilities_all_class.csv", index=False)
        ...
        output_predict_file = os.path.join(
            training_args.output_dir, f"predictions.txt")
